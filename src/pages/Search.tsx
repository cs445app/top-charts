...
// type SearchResult = [string, string, string, string];
// type SearchProps = {};
...
//   const [shouldDisplayElements, setShouldDisplayElements] = useState<boolean>(true);
...
//     const results = dataList.filter(row => row[1].toLowerCase().includes(query.toLowerCase()));
...
//     const streams = streamString.match(/\d+/g)?.join(',');
...
//           datasets: [{
//             label: 'Total Streams',
//             data: selectedStreams.split(',').map(parseFloat).concat(20093095926.00),
//             backgroundColor: ['#C850C0', '#4158D0']
//           }]
//         }
//       });
//     } else {
...
//     if (ctx) {
//       const colors = generateRandomColors(searchResults.length); // Generate colors dynamically
//       new Chart(ctx, {
//         type: 'bar',
//         data: {
//           labels: searchResults.map(result => result[2]), // Song titles as labels
//           datasets: [{
//             label: 'All songs in the top 200!',
//             data: searchResults.map(result => parseFloat(result[3])), // Number of streams as data
...
//       const color = `rgb(${Math.floor(Math.random() * 256)}, ${Math.floor(Math.random() * 256)}, ${Math.floor(Math.random() * 256)}, 0.9)`;
//       colors.push(color);
//     }
//     return colors;
//   };
...
//     setSearchResults(dataList); // Set search results to include all items from dataList
...
//               <div className='chart-title'>Top Chart's Graphs</div>
//               <div>Pie chart</div>
//               <canvas id="pieChart" className="chart-canvas"></canvas>
//               <canvas id="barGraph" className="bar-graph"></canvas>
...
const Search: React.FC<SearchProps> = () => {
  ...
  const dataList: SearchResult[] = [
    ['1', 'Taylor Swift', 'Cruel Summer', '1447105643'],
    ['2', 'Benson Boone', 'Beautiful Things', '333252910'],
    ['3', 'Drake', 'One Dance', '2224714216'],
    ['4', 'The Weeknd', 'One Of The Girls', '528411083'],
    ['5', 'Jack Harlow', 'Lovin On Me', '505542762'],
    ['6', 'Teddy Swims', 'Lose Control', '340622840'],
    ['7', 'Ariana Grande', 'we cant be friends (wait for your love)', '90071696'],
    ['8', 'FloyyMenor', 'Gata Only', '111270142'],
    ['9', '¥$', 'CARNIVAL', '190014759'],
    ['10', 'Mitski', 'My Love Mine All Mine', '688471096'],
    ['11', 'Beyoncé', 'TEXAS HOLD EM', '175482749'],
    ['12', 'Noah Kahan', 'Stick Season', '473059111'],
    ['13', 'Xavi', 'La Diabla', '445897476'],
    ...
    ['22', 'Arctic Monkeys', 'I Wanna Be Yours', '1442852469'],
    ['23', 'Natanael Cano', 'Madonna', '24007477'],
    ['24', 'Tate McRae', 'greedy', '926565731'],
    ['25', 'Natasha Bedingfield', 'Unwritten', '177419062'],
    ['26', 'Billie Eilish', 'What Was I Made For? [From The Motion Picture ""Barbie""]', '703540573'],
    ['27', 'Dua Lipa', 'Houdini', '329973788'],
    ['28', 'Lord Huron', 'The Night We Met', '738305840'],
    ...
    ['74', 'LE SSERAFIM', 'Smart', '43054390'],
    ['75', 'Frank Ocean', 'Pink + White', '427479710'],
    ['76', 'Djo', 'End of Beginning', '157675860'],
    ['77', 'David Guetta', 'Im Good (Blue)', '1495535879'],
    ['78', 'The Neighbourhood', 'Sweater Weather', '2115885523'],
    ['79', 'Post Malone', 'Sunflower - Spider-Man: Into the Spider-Verse', '3059545540'],
    ['80', 'Coldplay', 'Yellow', '1426216995'],
  ...
  const displayPieChart = () => {
    ...
    if (ctx) {
      new Chart(ctx, {
        type: 'pie',
        data: {
          labels: ['Total Streams of Artist', 'Total Streams of Top 200 Artist'],
          datasets: [{
            label: 'Total Streams',
            data: selectedStreams.split(',').map(parseFloat).concat(20093095926.00),
            backgroundColor: ['#C850C0', '#4158D0']
  ...
  const displayBarGraph = () => {
    const ctx = document.getElementById('barGraph') as HTMLCanvasElement;
    if (ctx && selectedResult) {
      const artistName = selectedResult[1]; // Get the selected artist's name
      const artistSongs = dataList.filter(result => result[1] === artistName); // Filter all data for the selected artist
      const sortedSongs = artistSongs.sort((a, b) => parseFloat(b[3]) - parseFloat(a[3])); // Sort songs by stream counts
      const top5Songs = sortedSongs.slice(0, 5); // Get the top 5 songs
      const songs = top5Songs.map(result => result[2]); // Get the names of the top 5 songs
      const streamCounts = top5Songs.map(result => parseFloat(result[3])); // Get the stream counts of the top 5 songs
      const colors = generateRandomColors(songs.length); // Generate colors dynamically
      new Chart(ctx, {
        type: 'bar',
        data: {
          labels: songs, // Song titles as labels
          datasets: [{
            label: `All Songs in the top 200 by "${artistName}"`,
            data: streamCounts, // Number of streams as data
            backgroundColor: colors // Use dynamically generated colors
          }]
        },
        options: {
      ...
      console.error('Canvas element not found or selectedResult is null');
    }
  };
